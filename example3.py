...
'''
1. **起飞**：无人机解锁并起飞到10米的高度。
2. **移动**：
   - 向左飞行10米/秒，持续10秒。
   - 向前飞行10米/秒，持续10秒。
   - 向右飞行10米/秒，持续10秒。
   - 向后飞行10米/秒，持续10秒。
   这四个步骤结合起来，无人机飞出了一个正方形的路径。
3. **返回**：无人机接收到“返航”指令，自动返回到起飞点（home点）并自动降落。

代码中的`send_body_ned_velocity`函数控制无人机在机体坐标系中的速度，通过设置不同的x、y、z轴速度，无人机可以前后左右移动。由于无人机在每个方向上的速度和时间都是相同的，所以它飞出的路径是一个正方形。

最后，使用`simple_goto`函数让无人机飞向一个预设的GPS坐标点，然后通过设置无人机的模式为`VehicleMode("RTL")`来让无人机执行返航操作。这个操作会让无人机自动返回到起飞点并降落，确保了无人机的安全回收。
